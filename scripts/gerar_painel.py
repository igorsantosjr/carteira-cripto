...
def grafico_semanal(serie):
    ...
    return f"""<svg class="grafico" viewBox="0 0 640 190" preserveAspectRatio="xMidYMid meet" role="img"
...
<style>
  ...
  *{{box-sizing:border-box;}}
  ...
  .badge{{font-family:'JetBrains Mono',monospace;font-size:11px;padding:3px 8px;border-radius:5px;
    background:rgba(232,184,75,.14);color:var(--ambar);}}
...
{chr(10).join(linhas_mes)}
  ...
  <footer>
    Gerado automaticamente em {agora.strftime('%d/%m/%Y às %H:%M')} (horário de Brasília) &middot;
    cotações via CoinGecko / Crypto.com &middot; câmbio via {ctx['fonte_cambio']}<br>
